...
const listWorkoutDatesWithSetsSince = `-- name: ListWorkoutDatesWithSetsSince :many
...
type ListWorkoutDatesWithSetsSinceParams struct {
	UserID      int64
	WorkoutDate string
}
...
func (q *Queries) ListWorkoutDatesWithSetsSince(ctx context.Context, arg ListWorkoutDatesWithSetsSinceParams) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, listWorkoutDatesWithSetsSince, arg.UserID, arg.WorkoutDate)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&workout_date); err != nil {
			return nil, err
		}
		items = append(items, workout_date)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
